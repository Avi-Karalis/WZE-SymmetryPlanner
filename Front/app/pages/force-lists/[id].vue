<template>
	<div class="p-4 sm:p-6 max-w-7xl mx-auto">
		...
		<div v-if="loading && !forceList" class="text-center py-16 text-gray-400">
			Loading force list...
		</div>
		...
		<template v-if="forceList">
			...
			<div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between mb-4">
				<div class="flex-1 min-w-0">
					...
					<div class="text-sm text-gray-400 mt-1 flex flex-wrap gap-3">
						...
						<span>DP:
							<span :class="dpClass">{{ currentDp }} / {{ forceList.maxDp }}</span></span>
						<span>SP:
							<span :class="spClass">{{ spUsed }} / {{ spAvailable }}</span></span>
					</div>
				</div>
				<div class="flex gap-2 shrink-0">
					<button class="btn-validate flex-1 sm:flex-none" @click="runValidation" :disabled="validating">
			...
			<div class="w-full bg-gray-700 rounded-full h-2 mb-6">
				<div class="h-2 rounded-full transition-all" :class="dpBarClass" :style="{
					width:
			...
			<!-- Validation result -->
			<div v-if="validationResult" class="mb-6 rounded-lg p-4" :class="validationResult.isValid
				...
				<ul v-if="validationResult.errors?.length" class="list-disc list-inside text-sm space-y-1">
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<div>
					<h2 class="text-lg font-semibold mb-3">
						Roster
						<span class="text-gray-400 text-sm">({{ forceList.units?.length ?? 0 }} units)</span>
					</h2>

					<p v-if="!forceList.units?.length" class="text-gray-500 text-sm">
						No units added yet.
					</p>
					<div class="flex flex-col gap-2">
						<div v-for="unit in forceList.units" :key="unit.id" class="unit-card group" @click="
							selectedUnit =
							selectedUnit?.id === unit.id ? null : unit
							">
							<div class="flex items-center justify-between">
								<div>
									<span class="font-medium">{{ unit.faction }}
										{{ unit.unitType }}</span>
									<span class="ml-2 text-xs text-gray-400">{{
										unit.designation?.join(", ")
									}}</span>
									<span v-if="unit.designationTypeLimit"
										class="ml-1 text-xs text-yellow-500 dark:text-yellow-300">({{
											unit.designationLimitValue }}: {{ unit.designationTypeLimit }})</span>
								</div>
								<div class="flex items-center gap-3">
									<span class="text-xs bg-gray-300 dark:bg-gray-700 px-2 py-0.5 rounded">{{
										unit.dpCost }} DP</span>
									<span v-if="unit.spCost > 0"
										class="text-xs bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 px-2 py-0.5 rounded">+{{
											unit.spCost }} SP</span>
									<span v-if="unit.spCost < 0"
										class="text-xs bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300 px-2 py-0.5 rounded">{{
											-unit.spCost }} SP cost</span>
									<button
							...
							<div v-if="selectedUnit?.id === unit.id" class="mt-3 border-t border-gray-600 pt-3">
								<div class="grid grid-cols-5 sm:grid-cols-9 gap-1 text-center text-xs mb-2">
									<div v-for="stat in [
										'MV',
										'MW',
										'CC',
								...
								<div v-if="unit.weapons?.length" class="text-xs text-gray-600 dark:text-gray-300 mb-1">
								...
								<div v-if="unit.unitSpecialAbilities?.length"
									...
									<span class="text-gray-500 dark:text-gray-400">Abilities: </span>{{
					...
					<div class="flex flex-col gap-2 mb-4">
						<div v-for="asset in selectedAssets" :key="asset.id" class="unit-card group">
							<div class="flex items-center justify-between">
								<div>
								...
								<div class="flex items-center gap-2">
									...
									<button class="btn-sm btn-danger transition opacity-100 sm:opacity-0 sm:group-hover:opacity-100"
				...
				<div>
					...
					<div class="flex flex-col sm:flex-row gap-2 mb-3">
						<input v-model="unitSearch" class="field-input text-sm flex-1" placeholder="Search..." />
						<select v-model="filterDesignation" class="field-input text-sm sm:max-w-36">
					...
					<div class="flex flex-col gap-1 max-h-[60vh] overflow-y-auto pr-1">
						<div v-for="unit in filteredAvailable" :key="unit.id"
							class="unit-card-available group cursor-pointer" @click="addUnit(unit)">
							<div class="flex items-center justify-between">
								<div>
									<span class="font-medium text-sm">{{ unit.faction }}
										{{ unit.unitType }}</span>
									<span class="ml-2 text-xs" :class="allyBadgeClass(unit)">{{
										unit.designation?.join(", ")
									}}</span>
									<span v-if="unit.designationTypeLimit"
										class="ml-1 text-xs text-yellow-500 dark:text-yellow-300">({{
								...
								<div class="flex items-center gap-2">
									...
									<span v-if="unit.spCost < 0"
										class="text-xs bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-300 px-2 py-0.5 rounded">{{
											unit.spCost }} SP</span>
									<span class="text-xs text-green-400 opacity-0 group-hover:opacity-100 transition">+
										Add</span>
								</div>
							</div>
						</div>
						<p v-if="!loadingUnits && !filteredAvailable.length" class="text-gray-500 text-sm">
					...
					<h2 class="text-lg font-semibold mb-2">Available Assets</h2>

					<div class="flex flex-col gap-1">
						<div v-for="asset in availableAssets" :key="asset.id" class="unit-card-available cursor-pointer"
			...
			<div v-if="showRoster"
				...
				<div id="roster-print-area"
					...
					<div class="flex flex-col gap-6">
						<div v-for="unit in forceList.units" :key="unit.id"
							...
							<div class="flex items-center justify-between mb-3">
								<div>
									...
									<span class="ml-2 text-xs text-gray-500 dark:text-gray-400">{{
										unit.designation?.join(', ')
									}}</span> <span v-if="unit.designationTypeLimit"
										class="ml-1 text-xs text-yellow-600 dark:text-yellow-300">({{
											unit.designationLimitValue
										}}: {{ unit.designationTypeLimit }})</span>
								</div>
							...
							<div class="grid grid-cols-5 sm:grid-cols-9 gap-1 text-center text-xs mb-4">
								<div v-for="stat in ['MV', 'MW', 'CC', 'ST', 'DEF', 'AR', 'W', 'PW', 'LD']" :key="stat"
									class="bg-gray-100 dark:bg-gray-800 rounded py-1">
							...
							<div v-if="unit.weapons?.length" class="mb-3">
								...
								<div class="flex flex-col gap-2">
									<div v-for="w in unit.weapons" :key="w.id"
										...
										<div class="font-semibold mb-1">{{ w.name }}<span
												v-if="w.dynamicDAM || w.dynamicRange"
												class="ml-2 text-yellow-600 dark:text-yellow-300">
												({{ [w.dynamicDAM ? 'DynamicDAM' : '', w.dynamicRange ? 'Dynamic Range'
													:
													''].filter(Boolean).join('· ')
												}})</span>
										</div>
										<div class="flex flex-col gap-0.5">
											<template v-if="w.ccMod != null || w.ccDam != null">
												<div class="flex gap-4">
													<span><span class="text-gray-500 dark:text-gray-400">CC Mod
														</span>{{ w.ccMod ?? '—' }}</span>
													<span><span class="text-gray-500 dark:text-gray-400">CC DAM
														</span>{{ w.ccDam != null ? (w.dynamicDAM ? w.ccDam + unit.st :
															w.ccDam) : '—' }}</span>
													<span><span class="text-gray-500 dark:text-gray-400">Crit Fail
														</span>{{ w.critFail }}</span>
												</div>
											</template>
											<template v-if="w.shortRange != null">
												<div class="flex gap-4">
													<span><span class="text-gray-500 dark:text-gray-400">SR </span>{{
														w.dynamicRange ? w.shortRange + unit.st : w.shortRange }}</span>
													<span><span class="text-gray-500 dark:text-gray-400">SR Mod
														</span>{{ w.shortRangeMod ?? '—' }}</span>
													<span><span class="text-gray-500 dark:text-gray-400">SR DAM
														</span>{{ w.shortRangeDam != null ? (w.dynamicDAM ?
															w.shortRangeDam + unit.st : w.shortRangeDam) : '—' }}</span>
												</div>
											</template>
											<template v-if="w.longRange != null">
												<div class="flex gap-4">
													<span><span class="text-gray-500 dark:text-gray-400">LR </span>{{
														w.dynamicRange ? w.longRange + unit.st : w.longRange }}</span>
													<span><span class="text-gray-500 dark:text-gray-400">LR Mod
														</span>{{ w.longRangeMod ?? '—' }}</span>
													<span><span class="text-gray-500 dark:text-gray-400">LR DAM
														</span>{{ w.longRangeDam != null ? (w.dynamicDAM ?
															w.longRangeDam + unit.st : w.longRangeDam) : '—' }}</span>
												</div>
											</template>
											<template v-if="w.ccMod == null && w.ccDam == null">
												<div><span class="text-gray-500 dark:text-gray-400">Crit Fail </span>{{
													w.critFail }}</div>
											</template>
										</div>
										<div v-if="w.weaponSpecialAbilities?.length" class="mt-2">
											<div class="flex flex-wrap gap-1 mb-1">
												<span v-for="sa in w.weaponSpecialAbilities" :key="sa.id"
													class="sa-badge">
													{{ sa.name }}<span v-if="sa.valueX"
														class="text-yellow-600 dark:text-yellow-300 ml-1">({{ sa.valueX
														}})</span>
												</span>
											</div>
											<div v-for="sa in w.weaponSpecialAbilities" :key="'wdesc-' + sa.id"
												class="text-gray-500 dark:text-gray-400 italic"
												style="font-size:10px; line-height:1.3;">
												<span
													class="font-semibold not-italic text-gray-700 dark:text-gray-300">{{
														sa.name }}<span v-if="sa.valueX"> ({{ sa.valueX }})</span>:</span>
												{{
													sa.description }}
											</div>
										</div>
							...
							<div v-if="unit.unitSpecialAbilities?.length">
								...
								<div class="flex flex-wrap gap-1 mb-1">
									<span v-for="sa in unit.unitSpecialAbilities" :key="sa.id" class="sa-badge">
										{{ sa.name }}<span v-if="sa.valueX || sa.valueY"
											class="text-yellow-600 dark:text-yellow-300 ml-1">({{ [sa.valueX,
											sa.valueY].filter(Boolean).join(', ') }})</span>
									</span>
								</div>
								<div v-for="sa in unit.unitSpecialAbilities" :key="'desc-' + sa.id"
									class="text-gray-500 dark:text-gray-400 italic"
									style="font-size:10px; line-height:1.3;">
									<span class="font-semibold not-italic text-gray-700 dark:text-gray-300">{{ sa.name
									}}<span v-if="sa.valueX || sa.valueY"> ({{ [sa.valueX,
										sa.valueY].filter(Boolean).join(', ')
										}})</span>:</span> {{ sa.description }}
								</div>
							</div>
						</div>
					</div>
...
async function loadForceList() {
	...
	console.log(forceList.value)
	if (!forceList.value) loadError.value = 'Force list not found.'

}

const currentDp = computed(() => {
	const unitDp = forceList.value?.units?.reduce((s, u) => s + (u.dpCost ?? 0), 0) ?? 0
	const assetDp = selectedAssets.value?.reduce((s, a) => s + (a.dpCost ?? 0), 0) ?? 0
	return unitDp + assetDp
})
...
async function addAsset(asset) {
	try {
		await apiAddAsset(forceList.value.id, asset.id)
		await loadForceList()
		validationResult.value = null
	} catch (e) {
		alert('Failed to add asset: ' + (e.response?.data || e.message))
	}
}

async function removeAsset(asset) {
	try {
		await apiRemoveAsset(forceList.value.id, asset.id)
		await loadForceList()
		validationResult.value = null
	} catch (e) {
		alert('Failed to remove asset: ' + (e.response?.data || e.message))
	}
}
...
.unit-card {
	@apply bg-gray-100 border border-gray-300 text-gray-800 rounded-lg p-3 cursor-pointer hover:border-gray-400 transition dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100 dark:hover:border-gray-500;
}

.unit-card-available {
	@apply bg-gray-100 border border-gray-300 text-gray-800 rounded px-3 py-2 hover:border-green-500 transition dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100 dark:hover:border-green-600;
}
